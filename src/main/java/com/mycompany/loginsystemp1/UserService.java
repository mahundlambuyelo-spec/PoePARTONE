...
class UserService {
   ...
   boolean loginUser(String loginUsername, String loginPassword, com.mycompany.loginsystemp1.User registeredUser) {
    if (registeredUser == null) return false;
    return registeredUser.getPassword().equals(loginPassword)
        && registeredUser.getUsername().equals(loginUsername);
}
...
class User {
    ...
    // PASSWORD: 8+ chars, one capital, one number, one special character
    ...
    String registerUser(String username, String password, String cellPhone) {
        ...
        if (!isValidCellPhone(cellPhone)) {
            return "Cell phone number incorrectly formatted, please ensure the number is " +
                   "in the format +27xxxxxxxxx or 27xxxxxxxxx.";
        }

        return "Welcome " + username + ", it is great to see you.";
    }

    boolean loginUser(String loginUsername, String loginPassword, User registeredUser) {
        if (registeredUser == null) return false;
        return registeredUser.getUsername().equals(loginUsername)
            && registeredUser.getPassword().equals(loginPassword);
    }

    String getLoginMessage(boolean loginSuccess) {
        if (loginSuccess) {
